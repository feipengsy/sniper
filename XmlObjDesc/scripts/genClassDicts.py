      ...
      if cl.has_key('template'):
        for t in cl['template']:
          ...
          if tcname.lower() == 'vector':
            ...
            # include header file
            self.sIncludes = self.conc(self.sIncludes, '#include <list>')
            # include template instantiation
            self.sDictInstances = self.conc(self.sDictInstances, 'std::list<%s> m_std_list_%s;' % (t1name, self.clean(t1name)))
            # include element for selection file
            self.sClassSelections = self.conc(self.sClassSelections, '  <class name="std::list<%s>"/>' % t1name)
          elif tcname.lower() == 'map':
            t2name = t['attrs']['t2'].replace('THIS',clname)
            # include header file
            self.sIncludes = self.conc(self.sIncludes, '#include <map>')
            # include template instantiation
            self.sDictInstances = self.conc(self.sDictInstances, 'std::map<%s,%s> m_std_map_%s_%s;' % (t1name, t2name, self.clean(t1name), self.clean(t2name)))
            # include element for selection file
            self.sClassSelections = self.conc(self.sClassSelections, '  <class name="std::map<%s,%s>"/>' % (t1name,t2name))
            # include iterators and 'pairs' with maps
            self.sDictInstances = self.conc(self.sDictInstances, 'std::map<%s,%s>::iterator m_std_map_iterator_%s_%s;' % (t1name, t2name, self.clean(t1name), self.clean(t2name)))
            self.sClassSelections = self.conc(self.sClassSelections, '  <class name="std::map<%s,%s>::iterator"/>' % (t1name,t2name))
            self.sDictInstances = self.conc(self.sDictInstances, 'std::pair<%s,%s> m_std_pair_%s_%s;' % (t1name, t2name, self.clean(t1name), self.clean(t2name)))
            self.sClassSelections = self.conc(self.sClassSelections, '  <class name="std::pair<%s,%s>"/>' % (t1name,t2name))
            self.sDictInstances = self.conc(self.sDictInstances, 'std::pair<const %s,%s> m_std_pair_const_%s_%s;' % (t1name, t2name, self.clean(t1name), self.clean(t2name)))
            self.sClassSelections = self.conc(self.sClassSelections, '  <class name="std::pair<const %s,%s>"/>' % (t1name,t2name))
